...
- **Every change ships its test surfaces.** A PR that adds or changes a page, API route, or server action must update its entry in `tests/coverage-manifest.json` (allowed roles read from the code's real gates, never guessed) and ship the tests that entry points to. `npm run test:surfaces` runs in the CI `verify` job and fails on any unregistered, stale, or untested surface; the CI `e2e` job runs the full browser and API suite (every page × every role, every API route) against local Supabase on every PR. Both are blocking, including for small changes. See `docs/testing.md`.
