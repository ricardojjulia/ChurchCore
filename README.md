...
- Service planning now includes a song library and setlist builder — Story 1 of a 5-story split closing the top-ranked "no setlist builder, song library, or volunteer-role matching" competitive gap. Church-admin, pastor, and ministry-leader roles can search or create songs into a church-wide catalog, add them to a service plan, drag-and-drop reorder the setlist alongside the existing move-up/down controls, and see a non-blocking warning when a song was used within the church's configurable repeat window (default 12 weeks). Role taxonomy/team roster, rotation planning, and rehearsal scheduling remain future stories in the same split.
