...
**MVP readiness: 69/100** (Council Review 18, 2026-09-24) — up from the 67–69/100 band held since Council Review 15 (2026-09-22), which itself followed the first movement off 65/100 (Story 1, Council Review 14) after five consecutive unchanged rounds (Reviews 9–13). Council Review 18 adds no new user capability (it audited the E2E testing foundation, not a feature); the +1 reflects a real, blocking regression net over authorization and route contracts, and it surfaced six real bugs.
...
| # | Gap | Status | Estimate |
|---|---|---|---|
| 1 | Service planning depth — setlist builder, song library, volunteer-role matching for services | **~75% closed** (Council Review 15 Agent 4 estimate). Story 1 (song library + setlist builder) passed Council Review 14, 2026-09-22 ([PR #146](https://github.com/ricardojjulia/ChurchCore/pull/146), not yet merged to `main`). Story 2 (role taxonomy & team roster) passed Council Review 15, 2026-09-22, on `feat/service-planning-role-taxonomy` (stacked on `feat/service-planning-song-library`, no PR opened yet). Stories 3–5 (rotation planner, rehearsal scheduling, the `event_id`-required schema change) are not started. | 3–4 weeks total |
...
Sources: `docs/reviews/2026-09-17-council-review-9-agent-4-feature-competitive.md` (original ranking), `docs/reviews/2026-09-22-council-review-14-agent-4-feature-competitive.md` (Story 1 reassessment), `docs/reviews/2026-09-22-council-review-15-agent-4-feature-competitive.md` (latest reassessment, Story 2).
...
**Next**: Story B (the journeys the sweep doesn't drive — comms, child safety, service planning, pastoral, giving/finance, people) is the next E2E testing work, but only **after** the security follow-ups above (F7 `communication_logs` RLS, then webhook fail-open together with F4) — both are real access-control and production-safety gaps, not test-coverage gaps, and F4 was already queued ahead of F2 (Resend wiring) before this branch. Story 2 of the separate Service Planning split (Role Taxonomy & Team Roster, Council Review 15) is done. Before Story 3 (Rotation Planner) of the 5-story Service Planning split can start, both Story 1 ([PR #146](https://github.com/ricardojjulia/ChurchCore/pull/146)) and Story 2 (this branch's eventual PR, stacked on Story 1) need to actually merge to `main` — Story 3 will need Story 1+2's schema (`song_library`, `service_plan_role_types`). Stories 4–5 (rehearsal scheduling, and the precondition schema change making `event_id` required on `service_plans`) remain queued behind Story 3. The other Phase B blockers — mobile UX, recurring giving, migration tooling, provider breadth — are reconfirmed unchanged; service planning itself stays ranked #1 until the remaining stories land, since Council Review 15 judged Stories 1+2 combined only a ~75% close of that gap, not a full one. Residual, non-blocking backlog carried forward: nested route-segment `error.tsx` below `app/app`/`app/portal`/`app/control` and the still-unverified nested-`loading.tsx` gap in deeper member subroutes (both unchanged since Review 12/13); a pre-existing `SimpleGrid cols={3}` mobile-breakpoint issue at `volunteer-schedule.tsx:1402` (surfaced by Council Review 14, reconfirmed by Council Review 15, still outside any Story 1/2 diff); `lib/stripe/*` and `lib/volunteer-data.ts` remain untested (pre-existing, surfaced again by Council Review 14); missing contextual `aria-label`s on `RoleTypeManager`'s Edit/Deactivate buttons and no centralized skill catalog/governance table (both surfaced by Council Review 15). Do not restart Sprint 2–7 framing from scratch — most of that scope has already shipped under different names via the Council process.
