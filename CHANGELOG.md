...
- Added WS-C5 communications auto-retry queue and unsubscribe link injection. New `lib/communications/retry-eligible.ts` (`retryEligibleCommunications`) queries `communication_logs` for transient-failed rows (`status='failed', retry_count<3, error_code IN (timeout, rate_limited, provider_unavailable, network_error, temporary_failure)`), resolves recipient contacts, constructs a synthetic session, and calls `sendWithSuppression` for each row with conditional `WHERE retry_count < 3` update guards. New cron route `app/api/cron/communications-retry/route.ts` runs all-tenant retry every 15 minutes (mirroring the shepherd-ai cron auth pattern), returns HTTP 200 on full success and HTTP 207 when any retry still fails. `vercel.json` updated with `"*/15 * * * *"` schedule. `retryAllEligibleAction` added to `app/app/communications-actions.ts` for scoped church-level operator retry (pastor and church-admin roles only). `lib/notifications/queue-communication.ts` now injects an HMAC-signed unsubscribe footer into every immediate outbound email (plain-text body + HTML `<p>` tag) via `generateUnsubscribeLink`; throws before the adapter if `UNSUBSCRIBE_SECRET` is unset; `body_preview` in `writeLog` is taken from `input.body.slice(0,500)` before injection to keep audit log clean; scheduled sends are not injected. Tests: 44 new/extended across `lib/communications/retry-eligible.test.ts` (11), `app/api/cron/communications-retry/route.test.ts` (10), `app/app/communications-actions.test.ts` (+4 retryAllEligibleAction cases), `lib/notifications/queue-communication.test.ts` (8).
