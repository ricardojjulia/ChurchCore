...
- Added Stripe refund lifecycle (Phase C WS-C2): `refunded` and `partially_refunded` status on `event_registrations` and `event_registration_payments`, operator refund-initiation action (`initiateRegistrationRefundAction`), `charge.refunded` webhook handler with idempotency guard, inline refund panel with amount input and reason select in ChurchAdmin event workspace, and confirmation modal. Migration `supabase/migrations/20260601120000_stripe_refund_polish.sql` adds 5 refund columns and a sparse index on `refund_id`. Critical fix: `normalizeRegistrationPaymentStatus` now guards terminal refund states before the `amountPaidCents > 0` shortcut so refunded rows do not flip back to `paid` on re-render (`lib/event-registration-lifecycle.ts`, `lib/stripe/event-registrations.ts`, `app/app/church-admin-actions.ts`, `app/api/webhooks/stripe/route.ts`, `lib/church-admin-events-data.ts`, `components/application/church-admin-event-workspace.tsx`).
