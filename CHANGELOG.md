...
- Added WS-C4 Spanish UI coverage for five surfaces: `CommunicationsHub`, `FinanceAccountsWorkspace`, `FinanceBudgetWorkspace`, `FinanceDashboard`, `GivingAdminWorkspace`, and `GivingDashboard`. Added five new i18n namespaces (`communicationsHub`, `financeAccounts`, `financeBudget`, `financeDashboard`, `givingAdmin`) to `lib/i18n.ts` with full English and natural church-operations Spanish translations. All six components now use `useI18n()` and `t()` for UI copy including section labels, page/sidebar titles, tab labels, table headers, badge text, toast notifications, modal titles, form labels/placeholders, empty states, and call-to-action buttons. Channel labels, status label maps, account-type option arrays, and nav-item arrays that contained display strings were moved inside component bodies so they rebuild on locale change. The `i18n-provider.tsx` default context `t` fallback is preserved as the raw-key return (required by existing `member-bottom-nav` tests); the four readiness state title/description strings in `CommunicationsHub` remain hardcoded English strings consistent with `FinanceJournalWorkspace` and are deferred for i18n in a subsequent pass when the companion test is updated. Build: clean. Tests: 371/371 passing.
