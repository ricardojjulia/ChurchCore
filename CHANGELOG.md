...
- Added WS-C7 attendance CSV import backend: migration `supabase/migrations/20260605000000_ws_c7_attendance_source_id.sql` adds `source_id` column and a partial unique index (`attendance_source_id_church_idx`) to `public.attendance` for idempotent re-imports (the `import` check_in_method value was already present from `20260527100000_member_mobile_checkin_foundation.sql`). Source-adapter normalizer `lib/attendance-import-source-adapters.ts` maps `generic_csv`, `planning_center`, and `breeze` column aliases to canonical fields (`sourceId`, `profileEmail`, `eventSourceId`, `checkedInAt`, `status`) with auto-generated `ATT-N` fallback IDs. Dry-run engine `lib/attendance-import-dry-run.ts` parses CSV, builds three indexes (attendance `source_id→id`, profiles `email→id`, events `source_id→id`) and a `Set` of existing DB present `profileId:eventId` pairs, classifies each row as `create`, `update`, `skip`, or `reject` in strict order: invalid status (non-null, not `present`/`absent`/`excused`) → reject; invalid ISO 8601 `checked_in_at` → reject; duplicate source ID in file → skip; in-file present dup → skip; DB present dup (new source_id only) → skip; existing source_id → update; else → create. Resolves profile and event IDs from their respective indexes with per-row unmatched warnings. Persists `import_batches` (`attendance_csv`, `dry_run_completed`) and per-row `import_batch_rows` records with dual Supabase + local DB fallback. Commit function inserts with `check_in_method='import'`, status defaulting to `'present'` and `checked_in_at` defaulting to `now()` SQL function, applies updates row-by-row with per-row error isolation, and marks the batch `committed` or `failed`. Server actions `app/app/church-admin/attendance/import/actions.ts` gate both dry-run and commit behind `church-admin` role, `hasTenantBackendEnv()`, and `session.source === 'supabase'` checks. Tests: 50 new tests across `lib/attendance-import-source-adapters.test.ts` (19), `lib/attendance-import-dry-run.test.ts` (23), and `app/app/church-admin/attendance/import/actions.test.ts` (8).
