...
- Fixed generic tenant provisioning against the current schema and hardened reruns: church setup fields now persist on `churches`; existing profile IDs and dependent records are preserved; cross-tenant email reuse is rejected before Auth mutation; existing passwords change only with an explicit reset flag; role accounts receive distinct password inputs; placeholder contact/directory data stays disabled; tenant memberships and control-plane routing are reconciled; and generated client seed records contain no plaintext credentials.
- Verified and completed implementation of the software-factory bootstrap, feedback/error-triage, and language-translation ruling documents. Added Claude PR-review parity, Codex/Gemini translation entrypoints, and ADR indexing; strengthened tenant and all seven expected control-plane RLS audits so unavailable databases and schema drift fail closed; added control-plane RLS CI plus live feedback integration coverage; and expanded demo-feedback route/UI tests with stale-record, outage, and mutation-failure behavior. A full browser walkthrough across public, platform-admin, ChurchAdmin, Secretary, Pastor, Ministry Leader, and Member surfaces found and fixed runtime defects in control feedback, Communications role/readiness handling, Children's Ministry preview reads, Member preview dates, browser-push hydration, error recovery, and obsolete Playwright login URL assertions. Reconciled the roadmap so completed Phase B and Phase C work is not restarted; the next gate is uncoached external evaluation.
