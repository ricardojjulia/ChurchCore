...
3. **Agent 2: "super-admin on tenant pages may land on `/sign-in` only locally."** Plausible, not verified. Both local stacks run on 127.0.0.1 and share the default `sb-127-auth-token` cookie name, so the local observation may not match production. The sweep's expectation is labelled as observed local behavior, and verifying it on a hosted preview is a follow-up.
