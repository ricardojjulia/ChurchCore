...
- **Reliable local demo:** `npm run setup:local`, `npm run dev`, `npm run smoke:local`, and `npm run test:e2e:readiness` must pass on a clean machine. The local smoke now includes the Daily Desk, ChurchAdmin readiness, and the current route targets linked from weekly readiness; the Playwright readiness check opens the same path in Chromium after browser sign-in and verifies tenant roles are denied from ChurchAdmin-only readiness targets.
...
- **ChurchAdmin weekly path:** `/app/church-admin/readiness` now provides the guided route through setup, people, event, children, volunteers, giving, communications, reports, and suggested workflows. Readiness links now open filtered target views for people, events, children's ministry, volunteers, giving/finance, communications, reports, draft journals, and suggested workflows. The first shared `ReadinessSummary` contract is now in place for status, severity, issue count, recommended action, target route/query, and completion state. Setup, account request, people/household, weekend event, children's ministry, volunteer schedule, giving/finance, communications, reports, and suggested workflow readiness now use module-owned builders. Settings, account approvals, people readiness filters, event roster review, volunteer service plans, giving/finance exceptions, and suggested workflows now share standardized completed, empty, no-backend, permission-denied, and validation-error target-state UI. Continue tightening each remaining target until every target route has a complete resolve flow.
...
- **Empty/error states:** every admin module should explain whether it has no data, no backend, or insufficient permission. The shared readiness target-state pattern now covers settings, account approvals, people, events, volunteers, giving/finance, and workflows; children, finance journals, communications, and reports remain for the sensitive-ops slice.
- **Role access audit:** verify ChurchAdmin, Secretary / Office Admin, Pastor, Ministry Leader, Member, and Control Plane cannot see each other's restricted data. The first browser denied-role check now covers Secretary, Pastor, Ministry Leader, and Member against ChurchAdmin-only readiness targets; Control Plane remains tied to the separate local control-plane demo follow-up.
...
- Add missing empty/error states for children, finance journals, communications, and reports.
