...
This document describes the first role-specific deep workspace in ChurchCore.
...
The ChurchAdmin home dashboard includes live aggregate tenant summaries, and the Care, Weekend, Communications, and Giving lanes now read live tenant data with preview fallback when no backend is configured. The `/app/church-admin/settings`, `/app/church-admin/people`, `/app/church-admin/accounts`, and `/app/church-admin/events/[id]` routes use real tenant data, but CSV import, automated host-based tenant routing, and richer communications workflows still need to be added.
...
This is the first point where ChurchCore starts to feel like one connected tenant app instead of several isolated role demos.
