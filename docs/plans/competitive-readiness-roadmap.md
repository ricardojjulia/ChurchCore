...
3. Add or align module loaders so setup, accounts, people, events, children, volunteers, giving, finance, communications, reports, and workflows can each produce readiness summaries. **In progress:** setup, account requests, people/households, weekend events, and volunteer schedules now have module-owned readiness builders in `lib/church-admin-readiness-modules.ts`. The existing ChurchAdmin readiness aggregate still composes children's ministry, giving/finance, and workflows directly. Communications and reports summaries still need module-owned loaders.
...
- Keep each module's data loader responsible for its own domain, while the readiness workspace composes summaries. The first splits move setup, account request, people/household, weekend event, and volunteer schedule summary construction into `lib/church-admin-readiness-modules.ts`.
