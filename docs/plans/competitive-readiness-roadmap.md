...
**Status:** Started. `ccm_services` now includes an explicit day check-in session lifecycle (`draft`, `enabled`, `paused`, `closed`) with optional start/end window and session token metadata. ChurchAdmin service detail now supports session enable/pause controls, and child check-in actions are now gated to open services with `enabled` day sessions.
