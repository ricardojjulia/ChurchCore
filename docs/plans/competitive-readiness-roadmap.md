...
6. Add empty, no-backend, permission-denied, validation-error, and completed states to every readiness target route. **Started:** `components/application/readiness-target-state.tsx` defines the shared target-state pattern; settings, people readiness filters, and giving/finance exceptions now show standardized target-state evidence and the Playwright readiness path requires it for those routes.
