...
3. Add or align module loaders so setup, accounts, people, events, children, volunteers, giving, finance, communications, reports, and workflows can each produce readiness summaries. **In progress:** setup, account requests, people/households, weekend events, children's ministry, volunteer schedules, and giving/finance now have module-owned readiness builders in `lib/church-admin-readiness-modules.ts`. The existing ChurchAdmin readiness aggregate still composes suggested workflows directly. Communications and reports summaries still need module-owned loaders.
