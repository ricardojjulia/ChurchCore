...
- Complete a human native-speaker linguistic review of the `es` (and, if onboarded to governance, `es-PR`) catalog via `lib/localization-governance`'s `requestReview()`/`submitReview()` flow. This is a separate, still-outstanding gate from coverage above — per ADR 0009 Decision 5, the `es` catalog was written by engineering and seeded as `validated`, not `approved`, for exactly this reason. A 2026-09-18 evaluation confirmed the "Current Spanish-Ready Surfaces" and "Remaining Surface Queue" above still accurately describe coverage, and a first-pass review fixed 7 objective accent/spelling errors in the existing catalog, but the actual human review this checklist item calls for has not happened yet. See `docs/reviews/2026-09-18-spanish-translation-evaluation.md` and `docs/reviews/2026-09-18-spanish-catalog-linguistic-review.md`.
