...
```
Phase 1 — Accounts
  [ ] Supabase Pro — tenant project
  [ ] Supabase Pro — control-plane project
  [ ] Vercel Pro
  [ ] Verify cron jobs active

Phase 2 — Security
  [ ] C-3: Column-scope member RLS (+ C-2 resolved)
  [ ] H-4: Make consent_logs append-only
  [ ] H-1: Encrypt pastoral notes at rest
  [ ] H-5: Add audit_events table + triggers
  [ ] M-2: Implement right-to-erasure action
