...
- **F1 (compliance, must precede F2) — done, Council Review 17 (2026-09-23, `fix/comms-cron-consent-suppression-lookups`, commit `56247ae`).** `findSuppression`, `checkOptIn`, `writeLog`, `writeSuppressedLog` now run through the admin client, scoped by `church_id` (ADR 0022). Council Review 17 then found and fixed a related HIGH-severity issue (`queueCommunicationAction` and `lib/actions/audit.ts` were unauthenticated `"use server"` exports) and corrected this doc's/ADR 0022's own claim that scheduled broadcasts reached unsubscribed addresses — they actually reached nobody, since `resolveRecipients` was also anon-only; that's now fixed too (P2). See [docs/factory-runs/2026-09-23-comms-cron-consent-suppression-council-review-17.md](2026-09-23-comms-cron-consent-suppression-council-review-17.md).
