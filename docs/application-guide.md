...
Events can be bulk-imported from CSV files exported from Planning Center, Breeze, or any generic CSV source. The import flow runs a dry-run classification first (showing `create`, `update`, `skip`, and `reject` rows with reasons) before committing. Each row is matched to an existing event by `source_id` for idempotent re-imports. Rows are rejected for missing title, missing or non-ISO 8601 `starts_at` or `ends_at`, `ends_at` not after `starts_at`, invalid `approval_status` (allowed values: `draft`, `pending`, `approved`, `archived`), or invalid capacity (must be a positive integer). Ministry names are resolved case-insensitively against existing church ministries; unmatched ministry names are flagged in the dry-run preview and the event is imported without a ministry link. `approval_status` defaults to `draft` when absent. The import entry point for UI wiring is `/app/church-admin/events/import`.

Attendance records can be bulk-imported from CSV files exported from Planning Center, Breeze, or any generic CSV source. The import flow runs a dry-run classification first before committing. Each row is matched to an existing attendance record by `source_id` for idempotent re-imports. Rows are rejected for invalid `status` (allowed values: `present`, `absent`, `excused`) or a non-ISO 8601 `checked_in_at`. The dry-run deduplicates both within-file and against existing DB records: if two rows in the same file resolve to the same profile and event with status `present`, the second is skipped; if a new source ID already resolves to an existing present pair in the database, the row is also skipped. Profile emails are matched case-insensitively against existing church profiles and event source IDs are matched against event `source_id` values; unmatched references are flagged in the dry-run preview and the record is imported without the respective foreign key. `status` defaults to `present` and `checked_in_at` defaults to the commit time when absent. All imported records are written with `check_in_method = 'import'`. The import entry point for UI wiring is `/app/church-admin/attendance/import`.
